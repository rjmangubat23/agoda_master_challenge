...
public class NewsListAdapter extends ArrayAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        Result newsEntity = null;
        List<MediaEntity> mediaEntityList;
        MediaEntity mediaEntity;
        String thumbnailURL = "";

        try {
            newsEntity = (Result) getItem(position);
            mediaEntityList = newsEntity.getMultimedia();
            if (position < mediaEntityList.size()) {
                mediaEntity = mediaEntityList.get(position);
        ...
        viewHolder.newsTitle.setText(newsEntity != null ? newsEntity.getTitle() : null);
